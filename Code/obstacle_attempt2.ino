...
void checkObstacle(int currentNode, int nextNode, int destination) {
  ...
  if (distance > 3000) {
    ...
    rerouteObstacle(currentNode,nextNode,destination);
    innovation();
    turnAround();
  }
}

void rerouteObstacle(int blocked_node1, int blocked_node2, int destination){
  ...
  findOptimalRoute(currentNode, originalPath[1]);
  currentNode = optimizedPath[0];
  
  pathOptimized = true;

  // Print the optimized route for debugging
  Serial.println("\New route");
  findShortestPath(currentNode, destination, newpath, newpathLength)
