...
// Function to convert position from String to int
int getPositionFromString(String positionStr) {
  // Try converting the String to an int
  int position = positionStr.toInt();

  // If the conversion failed, toInt() will return 0, so we check if the positionStr is not "0"
  if (position == 0 && positionStr != "0") {
    Serial.println("Error: Invalid position string.");
    return -1;  // Return -1 if conversion fails
  }

  return position;  // Return the valid integer position
}
...
// Function to send a GET request with position
void getRoute(int position) {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("Wi-Fi disconnected. Reconnecting...");
    connectToWiFi();
  }

  String getRequest = "position=" + String(position);
  ...
  if (client.connect(server, port)) {
    ...
    // Change POST to GET and use the correct endpoint
    client.println("GET /api/getRoute/dggb1873 HTTP/1.1");
    client.println("Host: " + String(server));
    client.println("Content-Type: application/x-www-form-urlencoded");
    client.print("Content-Length: ");
    client.println(getRequest.length());
    client.println();
    client.println(getRequest);  // Send the request data
...
// Main function to handle route navigation
void navigateRoute() {
  ...
  while (true) {  // Keep looping until we confirm reaching node 5
    getRoute(position);  // Send the current position to the server
